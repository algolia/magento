...
permalink: /doc/m1/faq-support-data/
...
- The queue is more **reliable**. For 2 reasons. (1) Because if the job fails, the next run of the queue will automatically retry. (2) Because of how the the queue breaks up your index - see [below](/magento/doc/m1/faq-support-data/#my-data-is-too-large).
...
### Too many products
As already stated, [Algolia only accepts 10K index sizes](/magento/doc/m1/faq-support-data/#my-data-is-too-large). If you are not using the queue, there is no check on this, and so if the size of your products index exceeds 10K, the indexing will fail. With queue enabled, the cron job will break down the index into 10K chunks, thereby ensuring success.
